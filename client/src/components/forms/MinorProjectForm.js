...
const MinorProjectForm = ({ studentId, onSuccess, canDelete = true }) => {
  ...
  const fetchEntries = async () => {
    try {
      const r = await getMinorProjects(studentId);
      setEntries(r.data);
    } catch (e) {
      console.error(e);
    }
  ...
  return (
    <Stack gap="lg">
      <Card shadow="sm" padding="lg" radius="md" withBorder>
        ...
        >
          <Text size="sm">
            <strong>Marks:</strong> Based on project uniqueness, industry
            relevance, and implementation quality.
          </Text>
        </Alert>
          ...
          <Paper p="md" withBorder mb="lg">
            <form onSubmit={handleSubmit}>
              <Stack gap="md">
                ...
                <Group grow>
                  <TextInput
                    label="Industry/NGO/Community"
                    value={formData.industry_ngo_community}
                    onChange={(e) =>
                      setFormData({
                        ...formData,
                        industry_ngo_community: e.target.value,
                      })
                    }
                    placeholder="Associated organization"
                  />
                  <NumberInput
                    label="Uniqueness Score (1-10)"
                    required
                    value={formData.uniqueness_score}
                    onChange={(v) =>
                      setFormData({ ...formData, uniqueness_score: v })
                    }
                    min={1}
                    max={10}
                  />
              ...
              <Paper key={entry.id} p="md" withBorder>
                <Group justify="space-between" align="flex-start">
                  <Stack gap="xs" style={{ flex: 1 }}>
                    ...
                    <Group gap="xl">
                      <Text size="sm">
                        <strong>Uniqueness:</strong> {entry.uniqueness_score}/10
                      </Text>
                      {entry.industry_ngo_community && (
                        <Text size="sm">
                      ...
                      {entry.github_link && (
                        <Text size="sm">
                          <strong>GitHub:</strong> {entry.github_link}
                        </Text>
                      )}
                    </Group>
                  </Stack>
