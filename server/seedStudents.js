...
async function seedDatabase() {
  ...
  for (const student of students) {
    try {
      ...
      console.log(`✅ ${successCount + 1}. Added: ${student.student_name} (${student.roll_number}) - Semester ${student.semester}`);
      successCount++;
    } catch (error) {
      if (error.message.includes('UNIQUE constraint failed')) {
        console.log(`⚠️  Skipped: ${student.student_name} (${student.roll_number}) - Already exists`);
      } else {
        console.log(`❌ Error adding ${student.student_name}: ${error.message}`);
      }
      errorCount++;
    }
  }
