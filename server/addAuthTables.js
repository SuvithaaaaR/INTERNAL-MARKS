  ...
  db.run(
    ...
    (err) => {
      if (err) {
        ...
        console.log(
          "✓ Default faculty user created (username: faculty, password: admin123)",
        );
      }
    },
  );
         ...
         VALUES (?, ?, 'student', ?, ?, ?)`,
          ...
          (err) => {
            if (err && !err.message.includes("UNIQUE")) {
              console.error(
                `Error creating user for ${student.student_name}:`,
                err,
              );
            }
            ...
            if (completed === students.length) {
              console.log(
                `✓ Created student accounts for ${students.length} students`,
              );
              console.log("  Default password for all students: student123");
              console.log(
                "  Students login with their roll number as username",
              );
              ...
              db.close((err) => {
                if (err) {
                  ...
                  console.log("\nDefault Credentials:");
                  console.log(
                    "  Faculty - username: faculty, password: admin123",
                  );
                  console.log(
                    "  Students - username: [roll_number], password: student123",
                  );
                }
              });
            }
